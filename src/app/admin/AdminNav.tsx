...
export function AdminNav({ role, currentPath }: Props) {
  ...
  const navItems = [
    ...
    ...(['owner', 'supervisor', 'vendor', 'developer'].includes(role) ? [{ href: '/admin/proyectos', label: 'Proyectos' }] : []),
    ...(['owner', 'supervisor'].includes(role) ? [{ href: '/admin/brief-templates', label: 'Templates' }] : []),
