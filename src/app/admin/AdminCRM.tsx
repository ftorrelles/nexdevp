...
export function AdminCRM({ leads: initialLeads }: { leads: Lead[] }) {
  ...
  async function saveNota(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        setLeads((prev) =>
          prev.map((l) => (l.id === id ? { ...l, notas: notaValue } : l))
        )
      }
    } finally {
      setUpdating(null)
      ...
      <main className="px-6 py-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div
              key={stat.label}
              className="bg-nex-dark border border-white/10 rounded-xl p-5"
            >
            ...
            <table className="w-full text-sm font-jost">
              <thead>
                <tr className="border-b border-white/10">
                  {['Fecha', 'Nombre', 'Email', 'Teléfono', 'Tipo Negocio', 'Canal', 'Estado', 'Contactar', 'Notas'].map(
                    (col) => (
                      <th
                        key={col}
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="px-4 py-8 text-center text-nex-grey"
                    >
                  ...
                  filtered.map((lead) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-nex-white font-medium">
                        {lead.nombre}
                      </td>
                        ...
                        >
                          {ESTADO_OPTIONS.map((opt) => (
                            <option
                              key={opt}
                              value={opt}
                              className="bg-nex-dark text-nex-white"
                            >
                              {opt}
                            </option>
                          ))}
